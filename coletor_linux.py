...
PRO_CHECKLIST_URL = f"{PRO_URL}/operational/checklist-results"
...
class ProManservCollector:
    ...
    def login(self):
        ...
        try:
            ...
            if csrf_token:
                ...
                logger.info("Token CSRF encontrado para login")
            else:
                # Tentar encontrar em meta tag
                meta_csrf = soup.find('meta', {'name': 'csrf-token'})
                if meta_csrf:
                    login_data['_token'] = meta_csrf.get('content')
                    logger.info("Meta CSRF encontrado para login")
                else:
                    logger.warning("Nenhum token CSRF encontrado na página de login")
    ...
    def get_checklist_data(self):
        ...
        try:
            ...
            # Usar período fixo com dados conhecidos (janeiro 2024)
            data_inicio = "01/01/2024"
            data_fim = "31/01/2024"
            
            logger.info(f"Buscando dados de {data_inicio} até {data_fim}")
            
            # Parâmetros para filtrar (usando GET)
            filter_params = {
                'id': '',  # ID vazio para buscar todos
                'beginDate': data_inicio,
                'endDate': data_fim,
                'checklistId': '',  # Vazio para todos os checklists
                'userId': '',  # Vazio para todos os usuários
                'assetId': ''  # Vazio para todos os ativos
            }
            
            logger.info("Aplicando filtros...")
            
            # Aplicar filtros usando GET
            filter_response = self.session.get(PRO_CHECKLIST_URL, params=filter_params)
            filter_response.raise_for_status()
            
            # Fazer download do relatório
            logger.info("Fazendo download do relatório...")
            
            # Parse da página filtrada para obter token CSRF para export
            soup = BeautifulSoup(filter_response.text, 'html.parser')
            csrf_token = soup.find('input', {'name': '_token'})
            
            if not csrf_token:
                # Tentar buscar em meta tag
                csrf_meta = soup.find('meta', {'name': 'csrf-token'})
                if csrf_meta:
                    csrf_token_value = csrf_meta.get('content')
                    logger.info("Meta CSRF encontrado para export")
                else:
                    logger.error("Token CSRF não encontrado para export")
                    return None
            else:
                csrf_token_value = csrf_token.get('value')
                logger.info("Input CSRF encontrado para export")
            
            # Dados para exportação (usando POST)
            export_data = filter_params.copy()
            export_data['format'] = 'excel'
            export_data['_token'] = csrf_token_value
            
            # Headers específicos para a requisição de export
            export_headers = {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Referer': PRO_CHECKLIST_URL
            }
            
            download_response = self.session.post(PRO_EXPORT_URL, data=export_data, headers=export_headers)
            
            logger.info(f"Download status: {download_response.status_code}")
            logger.info(f"Content-Type: {download_response.headers.get('Content-Type', 'Unknown')}")
            logger.info(f"Content-Length: {len(download_response.content)}")
            logger.info(f"First 200 chars: {download_response.content[:200]}")
            ...
            if download_response.status_code == 200:
                ...
                # Também salvar como texto para análise
                text_file = PASTA_DOWNLOAD / f"checklist_{timestamp}.txt"
                with open(text_file, 'w', encoding='utf-8') as f:
                    try:
                        f.write(download_response.content.decode('utf-8'))
                    except:
                        f.write(str(download_response.content))
                
                logger.info(f"Arquivo baixado: {temp_file}")
                logger.info(f"Arquivo texto salvo: {text_file}")
                return temp_file
            else:
    ...
    def process_excel_file(self, file_path):
        ...
        try:
            ...
            logger.info(f"Tipo do arquivo: {type(file_path)}")
            
            if file_path is None:
                raise Exception("Caminho do arquivo é None")
    ...
    def run(self):
        ...
        try:
            ...
            # Limpeza (comentado para debug)
            # self.cleanup_downloads()
            logger.info("Pasta downloads mantida para análise")
