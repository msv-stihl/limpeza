...
class GitManager:
    def __init__(self, repo_path=None):
        ...
        try:
            ...
            # Configurar remote para usar SSH se não estiver configurado corretamente
    ...
    def _configure_remote(self):
        """Configura o remote para usar SSH em vez de HTTPS com token"""
        try:
            if self.repo and 'origin' in [remote.name for remote in self.repo.remotes]:
                origin = self.repo.remote('origin')
                current_url = list(origin.urls)[0]
                
                # Se estiver usando HTTPS com token placeholder, mudar para SSH
                if 'your_github_token_here@github.com' in current_url or 'https://github.com' in current_url:
                    ssh_url = f"git@github.com:{self.github_repo}.git"
                    logger.info(f"Configurando remote para usar SSH: {ssh_url}")
                    origin.set_url(ssh_url)
                    logger.info("Remote configurado para SSH com sucesso")
                    
        except Exception as e:
            logger.warning(f"Erro ao configurar remote SSH: {e}")
    ...
    def init_repo(self):
        ...
        try:
            if not (self.repo_path / '.git').exists():
                ...
                # Configurar remote usando SSH
                if self.github_repo:
                    remote_url = f"git@github.com:{self.github_repo}.git"
                    self.repo.create_remote('origin', remote_url)
                    logger.info("Remote origin configurado com SSH")
    ...
    def push_changes(self, branch='master'):
    ...
    def pull_changes(self, branch='master'):
